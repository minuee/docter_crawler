...
module.exports = {
    ...
    $('.doctorList .dL_line .profile_box').each((index, item) => {
      ...
      if(doctorName && link){
        doctorArray.push({
          ...
          profileUrl : profileUrlTmp ? `https://anam.kumc.or.kr${profileUrlTmp}` : null
        });
      }
    });
  ...
  crwalingProcess03: async (url) => {
    ...
    try {
      ...
      console.log(`deptName: ${deptName}, doctorName: ${doctorName}, specialty: ${specialty}`)
      ...
      $('div.section.section02 > .doc_info01_table tbody tr').each((index, element) => {
        ...
        if(text) {
          console.log(`1 type: ${type}, date: ${targetDate}, text: ${text}`)
          jsonData.push({ type, date : targetDate, text, url:''});
        }
      })
      ...
      $('div.section.section02 .tab_ui ul li:nth-of-type(1) tbody tr').each((index, element) => {
        ...
        const text = $(element).find('td').text().trim();
        if(text) {
          console.log(`2 type: ${type}, date: ${targetDate}, text: ${text}`)
          jsonData.push({ type, date : targetDate, text, url:''});
        }
      })
      ...
      $('div.section.section02 .tab_ui .tab_cont ul li:nth-of-type(2) tbody tr').each((index, element) => {
        ...
        const text = $(element).find('td').text().trim();
        if(text) {
          console.log(`3 type: ${type}, date: ${targetDate}, text: ${text}`)
          jsonData.push({ type, date : targetDate, text, url:''});
        }
      })    
      ...
      $('div.section.section03 > .tab_ui ul li:nth-of-type(1) tbody tr').each((index, element) => {
        ...
        if(text) {
          console.log(`논문 type: ${type}, date: ${targetDate}, title: ${text}`)
          papers.push({ type, date  :targetDate, title:text, journal, url:''});
        }
      })
      ...
      $('div.section.section03 > .doc_info01_table tbody tr').each((index, element) => {
        ...
        if(text) {
          console.log(`4 type: ${type}, date: ${targetDate}, text: ${text}`)
          jsonData.push({ type : '수상', date: targetDate, text, url:''});
        }
      })
      ...
      $('div.section.section04 .doc_info01_table tbody tr').each((index, element) => {
        ...
        if(text) {
          console.log(`5 type: ${type}, date: ${targetDate}, text: ${text}`)
          jsonData.push({ type : '언론', date : targetDate, text, url});
        }
      })    
