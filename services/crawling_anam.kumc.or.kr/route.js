...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_anam.kumc.or.kr/controller`);
...
/**
 * @swagger
 *  /v1/c/anam.kumc.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [anam.kumc.or.kr-고대안암병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/healthcheck', async (req, res, next) => {
  ...
  const HOSPITAL_ID = 'H01KR-11000004';
...
router.post('/step01', async (req, res, next) => {
  const HOSPITAL_ID = 'H01KR-11000004';
  const HOSPITAL_NAME = '학교법인 고려중앙학원 고려대학교의과대학부속병원(안암병원)';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
