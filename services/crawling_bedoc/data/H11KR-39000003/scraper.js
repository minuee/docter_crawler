...
async function jejuNationalUnivScraper(doctorData) {
  ...
  try {
    const response = await page.goto(hospital_site, { waitUntil: 'networkidle2', timeout: 20000 });
    
    if (response.status() === 400) {
        console.log(`[WARN] Page for ${bedoc_doctorname} returned 400 Bad Request. Marking as isExist: false.`);
    ...
    // Handle cases where the page might not load, etc.
...
module.exports = jejuNationalUnivScraper;
