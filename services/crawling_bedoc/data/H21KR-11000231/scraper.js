...
async function hosanScraper(doctorData) {
  ...
  try {
    ...
    let isExist = false;
    $('.doctor-list-box').each(function() {
        const name = $(this).find('.doc-name').text().trim();
        if (name.includes(bedoc_doctorname)) {
            isExist = true;
