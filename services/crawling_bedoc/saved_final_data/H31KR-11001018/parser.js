...
(async () => {
    ...
    try {
        ...
        // Find the section for the specific doctor
        const doctorSection = $('div.widget._text_wrap:contains("김성윤")').closest('div.doz_row').nextAll().find('div._text_wrap:contains("한양대학교")').first().closest('main');

        if (doctorSection.length) {
            synthesizedData.profileUrl = doctorSection.find('img[src*="883bcd2b9b6d1.png"]').attr('src');

            const historyLines = [];
            doctorSection.find('div[id*="text_w20240108b0ed9198002d4"] p').each((i, el) => {
                historyLines.push($(el).text().trim());
            });
            ...
            historyLines.forEach(line => {
                const content = line.replace(/·/g, '').trim().replace(/"/g, '');
                if (!content) return;

                if (content.includes('대학교') || content.includes('Medical School') || content.includes('의대')) {
