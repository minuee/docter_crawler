...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_cbnuh.or.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-43000001';
...
/**
 * @swagger
 *  /v1/c/cbnuh.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [cbnuh.or.kr-충북대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.cbnuh.or.kr/prog/clnicDept/main/sub01_01_01/list.do`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let j = 0; j < _.size(SP1.data); j++) {
        ...
        console.log(`hid: ${HOSPITAL_ID}, deptName: ${element.deptName}, doctorName: ${element.doctorName}, url: ${element.url} profile_url: ${element.profile_url}`)
        //if ( element.deptName == "감염내과" && element.doctorName == "정혜원") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/cbnuh.or.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "충북대학교병원 정보를 가져와야 한다  "
 *      tags: [cbnuh.or.kr-충북대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
...
router.post('/treatise', async (req, res, next) => {
  ...
  for (let i = 0; i < loopSize; i++) {
    ...
    try{
      ...
      console.log(`crwalingtreatise erorr : ${e}`);
    }
