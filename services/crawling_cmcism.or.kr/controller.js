...
module.exports = {
  ...
  crwalingProcess01: async (r_url) => {
    console.log(`crwalingProcess01: ${r_url}`);
    ...
    $('table.MedicalTeam > tbody > tr').each((index, element) => {
      ...
      $(element).find('td').each((index2, element2) => {
        let deptName = $(element2).find('span').text().trim();
        console.log(`deptName: ${index} ${index2} ${deptName}`);
        let tmpLink = $(element2).find('a').attr('href').trim();
        console.log(`tmpLink: ${index} ${index2} ${tmpLink}`);

        if ( !functions.isEmpty(deptName) && !functions.isEmpty(tmpLink) ) {
          let tmpInfo = {
            deptName,
            tmpLink,
          };
