 ...
 *      tags: [cmcseoul.or.kr-카톨릭대서울성모병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/healthcheck', async (req, res, next) => {
  ...
  const HOSPITAL_ID = 'H01KR-11000013';
 ...
 *    post:
 *      summary: "1단계  조회"
 *      description: "카톨릭대서울성모병원 정보를 가져와야 한다  "
 *      tags: [cmcseoul.or.kr-카톨릭대서울성모병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
...
router.post('/step01', async (req, res, next) => {
  ...
  const HOSPITAL_ID = 'H01KR-11000013';
  const HOSPITAL_NAME = '카톨릭대서울성모병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (jsonData) {
      for (let ii = 0; ii < _.size(jsonData); ii++) {
        ...
        const element = jsonData[ii];
        console.log(`doctorName : ${element.drName}, deptName : ${element.drName}`)
        if ( element.drName == '이재호' && element.deptNm == '가정의학과' ) {
          await CS.wait(300);
          const detailLink = `https://www.cmcseoul.or.kr/api/doctor/${element.doctorDept.deptCd}/${element.drNo}`;
          const SP2 = await crawlingCtrl.get_rid_encrypt(element.drName, detailLink);
          if (SP2.error) {
          ...
          const SP3 = await crawlingCtrl.setCrawlingDoctorLink(tempRid, HOSPITAL_ID, element.deptNm, element.drName,detailLink,null,HOSPITAL_NAME );
          if (SP3.error) console.log("SP3 DB upsert fail.");
          data.push({
            hid: HOSPITAL_ID,
            deptName: element.deptNm,
            doctorName: element.drName,
            url:detailLink
          })
        }
      }
...
router.post('/step02', async (req, res, next) => {
  ...
  for (let i = 0; i < loopSize; i++) {
    ...
    if (doctorName && refUrl) {
      ...
      await CS.wait(300);
      for (let index = 0; index < _.size(SP1.data?.paper); index++) {
        const element = SP1.data?.paper[index];
        const iD = {
          rid: tempRid,
          title: element.title,
          doi: element.doi,
