...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_cnuh.com/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-46000001';
...
/**
 * @swagger
 *  /v1/c/cnuh.com/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [cnuh.com - 전남대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.cnuh.com/medical/info/dept.cs`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "간담췌외과" && SP1.data[i].doctorName == "김희준") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/cnuh.com/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "전남대학교병원 정보를 가져와야 한다  "
 *      tags: [cnuh.com - 전남대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
