...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_damc.or.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-48000002';
...
/**
 * @swagger
 *  /v1/c/damc.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [damc.or.kr-동아대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.damc.or.kr/02/02_2017.php`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "박주성") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/damc.or.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "동아대학교병원 정보를 가져와야 한다  "
 *      tags: [damc.or.kr-동아대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
