...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_dcmc.co.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-47000003';
...
/**
 * @swagger
 *  /v1/c/dcmc.co.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [dcmc.co.kr-대구카톨릭대병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-47000003';
  const HOSPITAL_NAME = '대구가톨릭대학교병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  const data = [];
  const r_url = `https://www.dcmc.co.kr/content/01reserv/01_01.asp`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "이건호") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/dcmc.co.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "대구카톨릭대병원 정보를 가져와야 한다  "
 *      tags: [dcmc.co.kr-대구카톨릭대병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
