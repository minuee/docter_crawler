...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_dkuh.co.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-44000001';
...
/**
 * @swagger
 *  /v1/c/dkuh.co.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [dkuh.co.kr - 단국대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-44000001';
  const HOSPITAL_NAME = '단국대학교의과대학부속병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  const data = [];
  const r_url = `https://www.dkuh.co.kr/html_2016/03/02.php`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        if ( SP1.data[i].deptName == "소화기내과" && SP1.data[i].doctorName == "김홍자") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/dkuh.co.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "단국대학교병원 정보를 가져와야 한다  "
 *      tags: [dkuh.co.kr - 단국대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
