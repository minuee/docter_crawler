...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_gilhospital.com/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-41000003';
...
/**
 * @swagger
 *  /v1/c/gilhospital.com/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [gilhospital.com-가천대길병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-41000003';
  const HOSPITAL_NAME = '의료법인 길의료재단 길병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  const data = [];
  const r_url = `https://www.gilhospital.com/web/www/-4`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "서희선") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/gilhospital.com/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "가천대길병원 정보를 가져와야 한다  "
 *      tags: [gilhospital.com-가천대길병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
