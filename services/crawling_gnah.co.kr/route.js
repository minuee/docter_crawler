...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_gnah.co.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-42000001';
...
/**
 * @swagger
 *  /v1/c/gnah.co.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [gnah.co.kr - 강릉아산병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.gnah.co.kr/kor/CMS/DeptMgr/list.do?mCode=MN021`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "오미경") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/gnah.co.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "강릉아산병원 정보를 가져와야 한다  "
 *      tags: [gnah.co.kr - 강릉아산병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
