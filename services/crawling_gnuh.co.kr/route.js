...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_gnuh.co.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-48000007';
...
/**
 * @swagger
 *  /v1/c/gnuh.co.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [gnuh.co.kr-경상국립대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.gnuh.co.kr/gnuh/treat/list.do?rbsIdx=54`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "내분비내과" && SP1.data[i].doctorName == "김수경") {
          data.push({
            hid: HOSPITAL_ID,
          ...
          console.log("SP1.data[i]",SP1.data[i]);
          await CS.wait(200);
...
/**
 * @swagger
 *  /v1/c/gnuh.co.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "경상국립대학교병원 정보를 가져와야 한다  "
 *      tags: [gnuh.co.kr-경상국립대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
