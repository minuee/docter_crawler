...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_hosp.ajoumc.or.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-41000008';
...
/**
 * @swagger
 *  /v1/c/hosp.ajoumc.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [hosp.ajoumc.or.kr-아주대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://hosp.ajoumc.or.kr/dept/deptList.do`;
  const P1 = await crawlingCtrl.crwalingProcess01(r_url);
  console.log("ddddd__Ddddx",r_url,_.size(P1?.data));
  ...
  for (let i = 0; i < _.size(P1.data) ; i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "김광민") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/hosp.ajoumc.or.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "아주대학교병원 정보를 가져와야 한다  "
 *      tags: [hosp.ajoumc.or.kr-아주대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
...
router.post('/treatise', async (req, res, next) => {
  ...
  const P1 = await crawlingCtrl.getCrawlingDoctorLink(HOSPITAL_ID);
  console.log(`total ssie: ${_.size(P1.data)}`)
  if (CS.isEmpty(_.size(P1.data))) { return res.json(TS.fail({ code: 'DATA_NULL', message: 'response data is null' })) }
...
/**
 * @swagger
 *  /v1/c/hosp.ajoumc.or.kr/treatise:
 *    post:
 *      summary: "논문 조회"
 *      description: "아주대학교병원 정보를 가져와야 한다  "
 *      tags: [hosp.ajoumc.or.kr-아주대학교병원]
