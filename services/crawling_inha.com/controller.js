...
module.exports = {
  ...
  crwalingProcess01: async (r_url) => {
    ...
    try {
      ...
      $('ul.dept-list').find('li').each((index, element) => {
        ...
        const deptName = $(element).find('div.text-ko > a').text() ? $(element).find('div.text-ko > a').text() : null;
        const tmpLinkDepthNo = $(element).find('div.active-menu > button:first-child').attr('onclick') ? $(element).find('div.active-menu > button:first-child').attr('onclick') : null ; 
        ...
        if ( linkDepthArray.length > 0 ) {
          const link = `https://www.inha.com/${linkDepthArray[1]?.replaceAll("'", "")}`;
  ...
  crwalingProcess02: async (link,deptName) => {
    ...
    $('ul.doctor-list li').each((index, element) => {
      const doctorName = $(element).find('div.infomation > a:first-child > span').text() ? $(element).find('div.infomation > a:first-child > span').text() : null;
      const detailLink = $(element).find('div.infomation > a').attr('href') ? $(element).find('div.infomation > a').attr('href') : null;
      let tmpLink = detailLink.replace("p_p_id=searchDoctor_WAR_bookingHomepageportlet&", "");
      const doctor = {
        doctorName,
        deptName,
        url: tmpLink,
  ...
  crwalingProcess03: async (url) => {
    ...
    try {
      ...
      $('div.table-story').find("table > caption:contains('학회/할술활동')").parents('table').find('tbody > tr').each((index, dtElement) => {
        ...
        const dtYearText = $(dtElement).find('th').text() ? $(dtElement).find('th').text() : '';
        const dtText = $(dtElement).find('td').text() ? $(dtElement).find('td').text() : ''
        console.log(`학회/할술활동: ${dtYearText} ${dtText}`);
        if ( !functions.isEmpty(dtText) ) {
          const tmpText = dtText.trim().replace(/\t/g, '').replace(/\n/g, '');
          item.biography.push({
            targetDate : dtYearText,
