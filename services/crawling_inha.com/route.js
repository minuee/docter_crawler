...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-41000004';
  const HOSPITAL_NAME = '인하대학교의과대학부속병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      ...
      if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "이연지") {
        data.push({
            hid: HOSPITAL_ID,
 ...
 *    post:
 *      summary: "2단계  조회"
 *      description: "인하대학교의과대학부속병원 정보를 가져와야 한다  "
 *      tags: [inha.com-인하대학교의과대학부속병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
