    ...
    try{
      ...
      if (!functions.isEmpty(SP1.data)) {
        for (let i = 0; i < _.size(SP1.data); i++) {
          //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "김아솔") {
            data.push({
              hid: HOSPITAL_ID,
      ...
      console.log(`crwalingProcess02 ${P1.data[i].link}, errr : ${error}`);
    }
  ...
  for (let i = 0; i < doctorLinkTotal ; i++) {
    ...
    if (doctorName && refUrl) {
      ...
      try{
        ...
        console.log(`detail_area02 errr : ${error}`);
      }
      
      //언론는 따로 진행
      try{
        const refUrl_press = refUrl.replace('detail_area01','detail_area03')
        SP1_press = await crawlingCtrl.crwalingProcess03_press(refUrl_press, SP1_book.data);
        await CS.wait(300);
      ...
      try{
        ...
        console.log(`get_rid_encrypt errr : ${error}`);
      }
