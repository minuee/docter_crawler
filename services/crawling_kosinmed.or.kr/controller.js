...
module.exports = {
  ...
  crwalingProcess01: async (r_url) => {
    ...
    try {
      ...
      $('#depart_contents').find('div.swiper-slide').each((index, element) => {

        const deptName = $(element).find('a').find('span').text()  ? $(element).find('a').find('span').text() : '';
        ...
        if ( !functions.isEmpty(tmpLink) && !functions.isEmpty(deptName) && tmpLink.indexOf('http') == -1) {
          const link = `https://www.kosinmed.or.kr${tmpLink}`;
  ...
  crwalingProcess02: async (tmpUrl,deptName) => {
    ...
    try {
      ...
      const doctors = [];
      $('div.tit-box').nextAll('div.staff-wrap').each((index, element) => {
        const doctorName = $(element).find('div.right-top').find('div.top > p.name > b').text() ? $(element).find('div.right-top').find('div.top > p.name > b').text().trim() : '';
        const detailLink = $(element).find('div.right-top').find('div.top').find('a').attr('href') ? $(element).find('div.right-top').find('div.top').find('a').attr('href') : '';
        ...
        if ( !functions.isEmpty(detailLink) ) {
          tmpLink =  `https://www.kosinmed.or.kr/depart/${detailLink}`;
        }
        if ( !functions.isEmpty(doctorProfileUrl) ) {
          tmpProfileUrl =  `https://www.kosinmed.or.kr${doctorProfileUrl}`;
        }
  ...
  crwalingProcess03: async (url) => {
    ...
    try {
      ...
      const htmlContent = await page.content();
      const $ = cheerio.load(htmlContent);  
      let tmpSpecialty = $('div.staff-wrap').find('div.clinic > dl').find('dd').text() ? $('div.staff-wrap').find('div.clinic > dl').find('dd').text().trim() : '';
