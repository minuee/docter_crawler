 ...
 *      tags: [mokdong.eumc.ac.kr-이대 목동병원]
 *      responses:
 *        "200":
 *          description: 접속 테스트
 *          content:
 ...
 *    post:
 *      summary: "1단계  조회"
 *      description: "이대 목동병원 정보를 가져와야 한다  "
 *      tags: [mokdong.eumc.ac.kr-이대 목동병원]
 *      produces:
 *      parameters:
 *        - name: "clientIp"
...
router.post('/step02', async (req, res, next) => {
  const ip = req.clientIp;
  const P1 = await ctrl.getCrawlingDoctorLink('H01KR-11000012');
  let totalCount = 0
  ...
  for (let i = 0; i < loopSize; i++) {
    ...
    if (doctorname && refUrl) {
      ...
      await CS.wait(300);
      const SP3 = await ctrl.setCrawlingdoctorBasic(tempRid, 'H01KR-11000012', SP1.data.basic.deptName, SP1.data.basic.doctorName, SP1.data.basic.specialty, SP1.data.basic.profileImgUrl);
      if (SP3.error) {
        console.log("SP3 DB fail.");
        return res.json(TS.fail("SP3 DB fail."));
      }
      ...
      await CS.wait(300);
      const SP4 = await ctrl.setCrawlingdoctorBiography(tempRid, 'H01KR-11000012', SP1.data.basic.doctorName, JSON.stringify(SP1.data.detail));
      if (SP4.error) {
