...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_mokdong.eumc.ac.kr/controller`);
...
/**
 * @swagger
 *  /v1/c/mokdong.eumc.ac.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [mokdong.eumc.ac.kr-이화여자대학교의과대학부속목동병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/healthcheck', async (req, res, next) => {
  ...
  const HOSPITAL_ID = 'H01KR-11000012';
  ...
  return res.send({
    'code': 200,
    'message': '이화여자대학교의과대학부속목동병원 접속테스트',
    'desc': 'success',
...
/**
 * @swagger
 *  /v1/c/mokdong.eumc.ac.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "이화여자대학교의과대학부속목동병원 정보를 가져와야 한다  "
 *      tags: [mokdong.eumc.ac.kr-이화여자대학교의과대학부속목동병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
...
router.post('/step01', async (req, res, next) => {
  const HOSPITAL_ID = 'H01KR-11000012';
  const HOSPITAL_NAME = '이화여자대학교의과대학부속목동병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
    ...
    if (element.link) {
      ...
      if (SP2.data) {
        ...
        for (let index = 0; index < _.size(SP2.data); index++) {
          ...
          // rid 만들기
          await CS.wait(300);
          console.log(`doctorName : ${element2.doctorName}, deptName : ${element2.deptName}, link : ${element2.link}`)
          if ( element2.doctorName !== '일반의' && element2.doctorName == '이상화' &&  element2.deptName == '가정의학과' ) {
            ...
            const hid = 'H01KR-11000012' // 이대
            if(tempRid){
              doctorCount++;
              console.log(`tempRid`, tempRid)
              await CS.wait(300);
              const SP4 = await crawlingCtrl.setCrawlingDoctorLink(tempRid, hid, element2.deptName, element2.doctorName, element2.link,element2.profileUrl,HOSPITAL_NAME)
              data.push({
                hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/mokdong.eumc.ac.kr/step02:
 *    post:
 *      summary: "2단계  조회"
 *      description: "이화여자대학교의과대학부속목동병원 정보를 가져와야 한다  "
 *      tags: [mokdong.eumc.ac.kr-이화여자대학교의과대학부속목동병원]
