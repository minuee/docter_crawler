...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_paik.ac.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-48000005';
...
/**
 * @swagger
 *  /v1/c/paik.ac.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [paik.ac.k-인제대부산백병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-48000005';
  const HOSPITAL_NAME = '인제대학교부산백병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  const data = [];
  const r_url = `https://www.paik.ac.kr/busan/user/department/list.do?menuNo=800035`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "박태진") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/paik.ac.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "인제대부산백병원 정보를 가져와야 한다  "
 *      tags: [paik.ac.k-인제대부산백병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
