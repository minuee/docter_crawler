...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_pnuyh.or.kr/controller`);
...
/**
 * @swagger
 *  /v1/c/pnuyh.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [pnuyh.or.kr-양산부산대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
/**
 * @swagger
 *  /v1/c/pnuyh.or.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "양산부산대학교병원 정보를 가져와야 한다  "
 *      tags: [pnuyh.or.kr-양산부산대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://www.pnuyh.or.kr/pnuyh/medical/department.do?tab=tab_d`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        //if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "이상엽") {
          data.push({
            hid: HOSPITAL_ID,
