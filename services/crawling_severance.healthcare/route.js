...
/**
 * @swagger
 *  /v1/c/severance.healthcare/step02_new:
 *    post:
 *      summary: "2단계  조회"
 *      description: "연대세브란스병원 정보를 가져와야 한다  "
 *      tags: [severance.healthcare-연대세브란스병원]
...
router.post('/step02_new', async (req, res, next) => {
  ...
  try{
    ...
    for (let i = 0; i < loopSize; i++) {
      ...
      const SP1 = await crawlingCtrl.crwalingProcess03_new(browser, P1.data[i].doctor_url);
      const doctorName = P1.data[i].doctorname;
      const deptName = P1.data[i].deptname;
      const refUrl = P1.data[i].doctor_url;
      const profileimgurl = P1.data[i].profileimgurl;
...
/**
 * @swagger
 *  /v1/c/severance.healthcare/treatise_new:
 *    post:
 *      summary: "3단계  조회"
 *      description: "연대세브란스병원 정보를 가져와야 한다  "
 *      tags: [severance.healthcare-연대세브란스병원]
