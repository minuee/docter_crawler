...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_yumc.ac.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-47000004';
...
/**
 * @swagger
 *  /v1/c/yumc.ac.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [yumc.ac.kr-영남대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const data = [];
  const r_url = `https://yumc.ac.kr/PageLink.do?link=yumc/08_minihp/contents_depart`;
  ...
  for (let i = 0; i < _.size(P1.data); i++) {
    ...
    if (!functions.isEmpty(SP1.data)) {
      for (let i = 0; i < _.size(SP1.data); i++) {
        if ( SP1.data[i].deptName == "가정의학과" && SP1.data[i].doctorName == "이근미") {
          data.push({
            hid: HOSPITAL_ID,
...
/**
 * @swagger
 *  /v1/c/yumc.ac.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "영남대학교병원 정보를 가져와야 한다  "
 *      tags: [yumc.ac.kr-영남대학교병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
