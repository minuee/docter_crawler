...
const crawlingCtrl = require(`${global.appRoot}/services/crawling_ywmc.or.kr/controller`);
...
router.post('/healthcheck', async function(req, res) {   
  const HOSPITAL_ID = 'H01KR-42000002';
...
/**
 * @swagger
 *  /v1/c/ywmc.or.kr/healthcheck:
 *    post:
 *      summary: "접속 테스트"
 *      description: "서버에 접속이 됬는데 "
 *      tags: [ywmc.or.kr - 연대원주 세브란스병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
 *          in: "body"
...
router.post('/step01', async function(req, res, next) {  
  ...
  const HOSPITAL_ID = 'H01KR-42000002';
  const HOSPITAL_NAME = '연세대학교 원주세브란스기독병원';
  const ret = await functions.checkHospitalId(HOSPITAL_ID, req, res);
  if ( ret.success === false ) {
    return res.send(ret);
  }
  ...
  const data = [];
  const r_url = `https://www.ywmc.or.kr/web/www/medical_office`;
...
/**
 * @swagger
 *  /v1/c/ywmc.or.kr/step01:
 *    post:
 *      summary: "1단계  조회"
 *      description: "연대원주 세브란스병원 정보를 가져와야 한다  "
 *      tags: [ywmc.or.kr - 연대원주 세브란스병원]
 *      produces:
 *      parameters:
 *        - name: "hid"
